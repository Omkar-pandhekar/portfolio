...
const CircularText: React.FC<CircularTextProps> = ({
  ...
  return (
    <motion.div
      initial={{ rotate: 0 }}
      className={`mx-auto rounded-full w-[150px] h-[150px] font-black text-center cursor-pointer origin-center bg-white dark:bg-zinc-900 ${className}`}
      animate={controls}
    ...
    >
      {letters.map((letter, i) => {
        ...
        return (
          <span
            key={i}
            className="absolute inline-block inset-2 text-sm transition-all duration-500 ease-[cubic-bezier(0,0,0,1)]"
